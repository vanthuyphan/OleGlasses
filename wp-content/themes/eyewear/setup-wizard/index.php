<?php
	...
	if ( ! class_exists( 'Hippo_Theme_Setup_Wizard' ) ):

		/**
		 * Hippo_Theme_Setup_Wizard class
		 */
		class Hippo_Theme_Setup_Wizard {

			private $portfolio_link   = 'http://themeforest.net/user/themehippo';
			private $support_link     = 'http://www.cloudsoftwaresolution.com/support/';
			private $docs_link        = 'http://www.cloudsoftwaresolution.com/documentation/wordpress/eyewear/';
			private $qs_pack_link     = 'http://www.cloudsoftwaresolution.com/support/want-to-full-demo-like-wordpress-install';
			...
			public function __construct() {
				...
				if ( apply_filters( $this->theme_name . '_theme_enable_setup_wizard', TRUE ) and current_user_can( 'manage_options' ) ) {
					$this->public_base_url = get_template_directory_uri() . '/setup-wizard';
			...
			public function admin_notices() {
				...
				?>
				<div class="updated">
					<p><?php printf( __( '<strong>Thank you for choosing the %1$s theme</strong> - You\'re almost ready to configure your site with %1$s theme :)', 'eyewear' ), $this->theme_nice_name ) ?></p>
					<p class="submit">
						<a href="<?php echo esc_url( admin_url( 'themes.php?page=' . $this->theme_name . '-setup' ) ) ?>"
						   class="button-primary"><?php printf( __( 'Run the %s Theme Setup Wizard', 'eyewear' ), $this->theme_nice_name ) ?></a>
						<a class="button-secondary skip"
						   href="<?php echo esc_url( wp_nonce_url( admin_url( '?hide-theme-setup-wizard=1' ), 'skip-theme-setup' ) ) ?>"><?php printf( 'Skip Setup', 'eyewear' ) ?></a>
					</p>
				</div>
				<?php
			}
			...
			public function admin_menus() {
				add_theme_page(
					sprintf( esc_html__( '%s Setup Wizard', 'eyewear' ), $this->theme_nice_name ),
					sprintf( esc_html__( '%s Setup Wizard', 'eyewear' ), $this->theme_nice_name ),
					'manage_options',
			...
			public function setup_wizard() {
				...
				$this->steps = array(
					'introduction' => array(
						'name'    => esc_html__( 'Introduction', 'eyewear' ),
				...
				if ( class_exists( 'TGM_Plugin_Activation' ) && isset( $GLOBALS[ 'tgmpa' ] ) ) {
					$this->steps[ 'default_plugins' ] = array(
						'name'    => esc_html__( 'Plugins', 'eyewear' ),
				...
				wp_localize_script( 'envato-setup', 'envato_setup_params', array(
					...
					'verify_text'      => esc_html__( '...verifying', 'eyewear' ),
				) );
		...
		public function setup_wizard_header() {
			...
			<head>
				...
				<<?php echo esc_attr( $tag ) ?>><?php esc_html_e( 'Theme &rsaquo; Setup Wizard', 'eyewear' ); ?>
				...
				<?php
					printf( '<a href="%s" target="_blank">%s</a>',
					        ...
					        sprintf(
						        ...
						        esc_url( eyewear_option( 'logo', 'url', get_template_directory_uri() . '/img/logo.png' ) ),
						        get_bloginfo( 'name' )
					        )
					);
				?>
			...
			<?php if ( 'next_steps' === $this->step ) : ?>
				<a class="wc-return-to-dashboard"
				   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'eyewear' ); ?></a>
			<?php endif; ?>
			</body>
			<?php
			...
			public function envato_setup_introduction() {
				if ( isset( $_REQUEST[ 'export' ] ) ) {
					...
					$ReduxFramework = ReduxFrameworkInstances::get_instance( eyewear_option_name() );
					...
					<p><a target="_blank" href="<?php echo esc_url( admin_url( 'export.php' ) ) ?>">Export Contents</a>
					</p>
					...
					?>
					<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s', 'eyewear' ), $this->theme_nice_name ); ?></h1>
					<p><?php printf( __( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. <br/><strong>It should only take 5 minutes.</strong>', 'eyewear' ), $this->theme_nice_name ); ?></p>
					<p><?php esc_html_e( "No time right now? If you don't want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!", 'eyewear' ); ?></p>
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'eyewear' ); ?></a>
						<a href="<?php echo esc_url( wp_get_referer() ? wp_get_referer() : admin_url( '' ) ); ?>"
						   class="button button-large"><?php esc_html_e( 'Not right now', 'eyewear' ); ?></a>
					</p>
					<?php
				}
			}
			...
			public function envato_setup_default_plugins() {
				...
				?>
				<h1><?php esc_html_e( 'Required Plugins', 'eyewear' ); ?></h1>
				<form method="post">
					...
					<?php
						...
						if ( count( $plugins[ 'all' ] ) ) {
							?>
							<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed:', 'eyewear' ); ?></p>
							...
							echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'eyewear' ) . '</strong></p>';
						} ?>

					<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.', 'eyewear' ); ?></p>

					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-large button-next"
						   data-callback="install_plugins"><?php esc_html_e( 'Continue', 'eyewear' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'eyewear' ); ?></a>
			...
			public function ajax_plugins() {
				if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST[ 'slug' ] ) ) {
					wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'eyewear' ) ) );
				}
				$json = array();
				...
				foreach ( $plugins[ 'activate' ] as $slug => $plugin ) {
					if ( $_POST[ 'slug' ] == $slug ) {
						$json = array(
							...
							'action2'       => - 1,
							'message'       => esc_html__( 'Activating Plugin', 'eyewear' ),
						);
						break;
					}
				}
				foreach ( $plugins[ 'update' ] as $slug => $plugin ) {
					if ( $_POST[ 'slug' ] == $slug ) {
						$json = array(
							...
							'action2'       => - 1,
							'message'       => esc_html__( 'Updating Plugin', 'eyewear' ),
						);
						break;
					}
				}
				foreach ( $plugins[ 'install' ] as $slug => $plugin ) {
					if ( $_POST[ 'slug' ] == $slug ) {
						$json = array(
							...
							'action2'       => - 1,
							'message'       => esc_html__( 'Installing Plugin', 'eyewear' ),
						);
						break;
					}
				}
				...
				if ( $json ) {
					...
					wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'eyewear' ) ) );
				}
				exit;
			...
			public function envato_setup_default_content() {
				?>
				<h1><?php esc_html_e( 'Demo Contents', 'eyewear' ); ?></h1>
				<form method="post">
					<p><?php esc_html_e( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue.', 'eyewear' ); ?></p>
					<table class="envato-setup-pages" cellspacing="0">
						<thead>
						<tr>
							<td class="check"></td>
							<th class="item"><?php esc_html_e( 'Item', 'eyewear' ); ?></th>
							<th class="description"><?php esc_html_e( 'Description', 'eyewear' ); ?></th>
							<th class="status"><?php esc_html_e( 'Status', 'eyewear' ); ?></th>
						</tr>
						</thead>
						<tbody>
					...
					<p><?php esc_html_e( 'Once inserted, this content can be managed from the WordPress admin dashboard.', 'eyewear' ); ?></p>

					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-large button-next"
						   data-callback="install_content"><?php esc_html_e( 'Continue', 'eyewear' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'eyewear' ); ?></a>
						<?php wp_nonce_field( 'envato-setup' ); ?>
					</p>
			...
			private function _content_default_get() {
				...
				$content[ 'data' ] = array(
					'title'            => esc_html__( 'Contents', 'eyewear' ),
					'description'      => esc_html__( 'This will create all demo data as seen in the demo.', 'eyewear' ),
					'pending'          => esc_html__( 'Pending.', 'eyewear' ),
					'installing'       => esc_html__( 'Installing contents.', 'eyewear' ),
					'success'          => esc_html__( 'Success.', 'eyewear' ),
					'install_callback' => array( $this, '_content_install_data' ),
				);

				$content[ 'menus' ] = array(
					'title'            => esc_html__( 'Menu', 'eyewear' ),
					'description'      => esc_html__( 'Insert default menu as seen in the demo.', 'eyewear' ),
					'pending'          => esc_html__( 'Pending.', 'eyewear' ),
					'installing'       => esc_html__( 'Installing Default Menu.', 'eyewear' ),
					'success'          => esc_html__( 'Success.', 'eyewear' ),
					'install_callback' => array( $this, '_content_install_menu' ),
				);

				$content[ 'widgets' ] = array(
					'title'            => esc_html__( 'Widgets', 'eyewear' ),
					'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.', 'eyewear' ),
					'pending'          => esc_html__( 'Pending.', 'eyewear' ),
					'installing'       => esc_html__( 'Installing Default Widgets.', 'eyewear' ),
					'success'          => esc_html__( 'Success.', 'eyewear' ),
					'install_callback' => array( $this, '_content_install_widgets' ),
				);

				$content[ 'theme_options' ] = array(
					'title'            => esc_html__( 'Options', 'eyewear' ),
					'description'      => esc_html__( 'Configure Default Theme Options.', 'eyewear' ),
					'pending'          => esc_html__( 'Pending.', 'eyewear' ),
					'installing'       => esc_html__( 'Installing default theme options.', 'eyewear' ),
					'success'          => esc_html__( 'Success.', 'eyewear' ),
					'install_callback' => array( $this, '_content_install_theme_options' ),
				);

				$content[ 'settings' ] = array(
					'title'            => esc_html__( 'Settings', 'eyewear' ),
					'description'      => esc_html__( 'Configure Reading Settings.', 'eyewear' ),
					'pending'          => esc_html__( 'Pending.', 'eyewear' ),
					'installing'       => esc_html__( 'Installing Default Reading Settings.', 'eyewear' ),
					'success'          => esc_html__( 'Success.', 'eyewear' ),
				...
				if ( class_exists( 'RevSlider' ) ) {
					$content[ 'revslider' ] = array(
						'title'            => esc_html__( 'Slider', 'eyewear' ),
						'description'      => esc_html__( 'Load slider Data.', 'eyewear' ),
						'pending'          => esc_html__( 'Pending.', 'eyewear' ),
						'installing'       => esc_html__( 'Importing revolution slider slide.', 'eyewear' ),
						'success'          => esc_html__( 'Success.', 'eyewear' ),
			...
			public function ajax_content() {
				...
				if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST[ 'content' ] ) && isset( $content[ $_POST[ 'content' ] ] ) ) {
					wp_send_json_error( array(
						                    'error'   => 1,
						                    'message' => esc_html__( 'No content Found', 'eyewear' )
					                    ) );
				}
				...
				if ( $json ) {
					...
					wp_send_json( array( 'error' => 1, 'message' => esc_html__( 'Error', 'eyewear' ) ) );
				}
			...
			public function envato_setup_logo_design() {
				...
				?>
				<h1><?php esc_html_e( 'Logo &amp; Design', 'eyewear' ); ?></h1>
				<form method="post">
					<p><?php printf( __( 'Please add your logo below. For best results, the logo should be a transparent PNG. The logo can be changed at any time from the <strong>%1$s Options &RightArrow; Logo Settings</strong> area in your <strong>%1$s Options</strong>.', 'eyewear' ), $this->theme_nice_name ); ?></p>
					...
					<table>
						<tr>
							...
							<td>
								<a href="#"
								   class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'eyewear' ); ?></a>
							</td>
						</tr>
					</table>


					<!--<p><?php /*_e( 'Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.', 'eyewear' ); */ ?></p>
				...
				<div class="theme-presets">
					...
					<p><em>Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and
							Web
					...
					<p class="envato-setup-actions step">
						<input type="submit" class="button-primary button button-large button-next"
						       value="<?php esc_attr_e( 'Continue', 'eyewear' ); ?>" name="save_step"/>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'eyewear' ); ?></a>
						<?php wp_nonce_field( 'envato-setup' ); ?>
					</p>
			...
			public function envato_setup_customize() {
				...
				admin. These include:', 'eyewear' ), $this->theme_nice_name ); ?>
				</p>

				<?php do_action( 'hippo_envato_setup_customize_features', $this ); ?>


				<p>To change the Sidebars go to <strong>Appearance &rightarrow; Widgets</strong>. Here widgets can be
					"drag
				...
				<?php if ( ! is_child_theme() ) { ?>
					<p>
						<em>Advanced Users: If you are going to make changes to the theme source code please use a <a
								href="https://codex.wordpress.org/Child_Themes" target="_blank">Child Theme</a> rather
							than
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'eyewear' ); ?></a>
				</p>
			...
			public function envato_setup_help_support() {
				...
				<p>This theme comes with 6 months item support from purchase date (with the option to extend this
					period).
					This license allows you to use this theme on a single website. Please purchase an additional license
					to
					use this theme on another website.</p>
				<p>Item Support can be accessed
					from <?php printf( '<a href="%1$s" target="_blank">%1$s</a>', $this->support_link ) ?> and
					includes:</p>
				<ul>
				...
				<ul>
					...
					<li>Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later
						on)
					</li>
				</ul>
				<p>More details about item support can be found in the ThemeForest <a
						href="http://themeforest.net/page/item_support_policy" target="_blank">Item Support Polity</a>.
				</p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'eyewear' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
				<?php
			}
			...
			public function envato_setup_ready() {
			   ...
			   data-via="EnvatoMarket" data-size="large">Tweet</a>', $this->portfolio_link, $this->theme_nice_name ); ?>
				...
				<h1><?php esc_html_e( 'Your Website is Ready!', 'eyewear' ); ?></h1>
				...
				<div class="envato-setup-next-steps">
					<div class="envato-setup-next-steps-first">
						<h2><?php esc_html_e( 'Next Steps', 'eyewear' ); ?></h2>
						<ul>
							<li class="setup-product">
								<a class="button button-primary  button-large"
								   href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'View your new website!', 'eyewear' ); ?>
								</a>
							</li>
							<li class="setup-product">
								<?php printf( '<a class="button button-next button-large"
					...
					<div class="envato-setup-next-steps-last">
						<h2><?php esc_html_e( 'More Resources', 'eyewear' ); ?></h2>
						<ul>
							...
							<li class="howto">
								<a href="https://wordpress.org/support/"
								   target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'eyewear' ); ?></a>
							</li>
							<li class="rating">
								<a href="<?php echo esc_url( $this->download_link ) ?>"
								   target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'eyewear' ); ?></a>
							</li>
