		...
		class Hippo_Menu_Walker extends Walker_Nav_Menu {
			...
			public static function fallback( $args ) {
				if ( current_user_can( 'manage_options' ) ) {
					$fb_output = '<li class="menu-item"><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'eyewear' ) . '</a></li>';
					$fb_output .= '<li class="menu-item"><a target="_blank" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">' . esc_html__( 'Add Menu', 'eyewear' ) . '</a></li>';
			...
			public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
				...
				if ( function_exists( 'hippo_plugin_get_menu_meta' ) ) {
					$load_sidebar      = hippo_plugin_get_menu_meta( $item->ID, 'widgets' );
					$load_column_class = hippo_plugin_get_menu_meta( $item->ID, 'menucolumnclass', 'col-md-10' );

					$menu_heading               = hippo_plugin_get_menu_meta( $item->ID, 'menuheading', '' );
					$menu_icon_color            = hippo_plugin_get_menu_meta( $item->ID, 'iconcolor', '' );
					$menu_icon_background_color = hippo_plugin_get_menu_meta( $item->ID, 'iconbackgroundcolor', '' );
					$menu_icon                  = hippo_plugin_get_menu_meta( $item->ID, 'icon', '' );
					$menu_background_image_id   = hippo_plugin_get_menu_meta( $item->ID, 'menubackgroundimage', '' );
				}
				...
				if ( $load_sidebar and is_active_sidebar( $load_sidebar ) ) {
					...
					$item_output .= '
                            ...
                            <ul class="label' . ( $depth + 1 ) . ' sub-dropdown-menu">
                                <li>
                                    <div class="megamenu-container ' . apply_filters( 'eyewear_megamenu_container_class', '' ) . '"><!-- .container class removed -->
                                        <div class="row">';
