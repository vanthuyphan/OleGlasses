...
class WC_CP_REST_API {
	...
	*/
		...
		if ( $product_id && 'composite' === $product_type ) {
			switch ( $field_name ) {
						...
						foreach ( $field_value as $data ) {
							...
							if ( 'create' === $action ) {
								...
								$component_data = array(
									...
									'options_style'         => isset( $data[ 'options_style' ] ) ? $data[ 'options_style' ] : 'dropdowns',
									'title'                 => isset( $data[ 'title' ] ) ? $data[ 'title' ] : ''
								);

								// Clean up source data.
								$data = array_diff_key( $data, array( 'id' => 1, 'query_ids' => 1, 'default_option_id' => 1, 'delete' => 1 ) );

								// Merge.
								$component_data = array_merge( $data, $component_data );
							...
							// Updating or deleting component.
								...
								// Updating component.
								$component_data = array_diff_key( $product->get_component( $component_id )->get_data(), array( 'composite_id' => 1, 'component_id' => 1 ) );
								...
								$query_type = isset( $data[ 'query_type' ] ) ? $data[ 'query_type' ] : $component_data[ 'query_type' ];

								$updated_data = array(
									'query_type'            => $query_type,
									'assigned_ids'          => 'product_ids' === $query_type && isset( $data[ 'query_ids' ] ) ? $data[ 'query_ids' ] : $component_data[ 'assigned_ids' ],
									'assigned_category_ids' => 'category_ids' === $query_type && isset( $data[ 'query_ids' ] ) ? $data[ 'query_ids' ] : $component_data[ 'assigned_category_ids' ],
									'default_id'            => isset( $data[ 'default_option_id' ] ) && '' !== $data[ 'default_option_id' ] ? $data[ 'default_option_id' ] : $component_data[ 'default_id' ],
									'priced_individually'   => isset( $data[ 'priced_individually' ] ) ? wc_bool_to_string( $data[ 'priced_individually' ] ) : $component_data[ 'priced_individually' ],
									'shipped_individually'  => isset( $data[ 'shipped_individually' ] ) ? wc_bool_to_string( $data[ 'shipped_individually' ] ) : $component_data[ 'shipped_individually' ],
									'optional'              => isset( $data[ 'optional' ] ) ? wc_bool_to_string( $data[ 'optional' ] ) : $component_data[ 'optional' ],
								);
								...
								// Merge.
								$updated_data = array_merge( $data, $updated_data );

								// Merge into component data.
								$component_data = array_merge( $component_data, $updated_data );
				...
				case 'composite_scenarios' :
					...
					$scenarios_rest_api_data = self::get_rest_api_scenario_data( $product->get_scenario_data( 'edit' ) );
					...
					if ( ! empty( $field_value ) && is_array( $field_value ) ) {
						foreach ( $field_value as $data ) {
							...
							if ( 'create' === $action ) {
								...
								// Validate.
								$scenario_data = self::validate_rest_api_scenario_data( $data );
							...
							// Updating or deleting scenario.
								...
								// Validate.
								$scenario_data = self::validate_rest_api_scenario_data( $scenario_data );
						...
						foreach ( $scenarios_rest_api_data_array as $scenario_id => $scenario_rest_api_data ) {
							...
							if ( ! empty( $scenario_rest_api_data[ 'configuration' ] ) ) {
								foreach ( $scenario_rest_api_data[ 'configuration' ] as $component_config_data ) {
							...
							if ( ! empty( $scenario_rest_api_data[ 'actions' ] ) ) {
								foreach ( $scenario_rest_api_data[ 'actions' ] as $action_data ) {
							...
							$scenarios_internal_data_array[ $scenario_id ] = array(
								'component_data'   => $component_data,
								'modifier'         => $modifier_data,
								'scenario_actions' => $actions_data,
								'description'      => $scenario_rest_api_data[ 'description' ],
								'title'            => $scenario_rest_api_data[ 'name' ]
							);
						}
					}
		...
		switch ( $key ) {
			...
			case 'composite_components' :
				...
				if ( 'composite' === $product_type ) {
					...
					/*
					 * Convert internal schema to REST API schema.
					 */
			...
			case 'composite_scenarios' :
				...
				if ( 'composite' === $product_type ) {
					$value = self::get_rest_api_scenario_data( $product->get_scenario_data( 'edit' ) );
				}
	 ...
	 * @return array
	 */
	private static function sanitize_rest_api_component_data( $component_data ) {
		...
		if ( ! empty( $component_data ) ) {
			foreach ( $component_data as $key => $value ) {
				...
				if ( in_array( $key, array( 'title', 'description', 'query_type', 'options_style' ) ) ) {
	 ...
	 * @return array
	 */
	private static function validate_internal_component_data( $component_data ) {
	 ...
	 * @return array
	 */
	private static function validate_rest_api_scenario_data( $data ) {
		...
		if ( ! empty( $data[ 'configuration' ] ) && is_array( $data[ 'configuration' ] ) ) {
			foreach ( $data[ 'configuration' ] as $component_config_data ) {
				...
				$validated_configuration[] = array(
					...
					'component_options' => array_map( 'absint', $component_config_data[ 'component_options' ] ),
	 ...
	 * @return array
	 */
	private static function get_rest_api_scenario_data( $internal_scenario_data ) {

		$converted = array();

		if ( ! empty( $internal_scenario_data ) && is_array( $internal_scenario_data ) ) {
			foreach ( $internal_scenario_data as $id => $data ) {
