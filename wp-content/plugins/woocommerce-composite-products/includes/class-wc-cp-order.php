					...
					if ( ! empty( $child_items ) ) {
						...
						foreach ( $child_items as $child_item_id => $child_item ) {
							...
							if ( 'no' === $child_item->get_meta( '_composite_item_needs_shipping', true ) ) {
								...
								if ( $component_title ) {
									$component_description = sprintf( __( '%1$s &ndash; Quantity: %2$s, SKU: %3$s', 'woocommerce-composite-products' ), $product_title, $child_item->get_quantity(), $sku );
								} else {
									$product_description = sprintf( __( 'Quantity: %1$s, SKU: %2$s', 'woocommerce-composite-products' ), $child_item->get_quantity(), $sku );
								}
