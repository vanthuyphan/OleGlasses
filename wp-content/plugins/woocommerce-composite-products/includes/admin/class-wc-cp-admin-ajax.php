		...
		if ( ! empty( $search_results ) ) {
			...
			foreach ( $search_result_objects as $product ) {
				if ( $product ) {
					...
					$search_results_incl_variations[ $product_id ] = WC_CP_Helpers::get_product_title( $product, $product->is_type( 'variable' ) ? sprintf( _x( '%s &ndash; Any Variation', 'any product variation', 'woocommerce-composite-products' ), $product->get_title() ) : '' );
