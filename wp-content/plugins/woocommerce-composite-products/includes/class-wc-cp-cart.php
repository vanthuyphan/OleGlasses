		...
		if ( $cart_item[ 'data' ]->needs_shipping() ) {
			...
			if ( $component_option && false === $component_option->is_shipped_individually() ) {
				...
				if ( apply_filters( 'woocommerce_composited_product_has_bundled_weight', false, $cart_item[ 'data' ], $component_id, $composite ) ) {
					$cart_item[ 'data' ]->composited_weight = (double) $cart_item[ 'data' ]->get_weight( 'edit' );
				}

				$cart_item[ 'data' ]->composited_value = (double) WC_CP_Core_Compatibility::get_prop( $cart_item[ 'data' ], 'price', 'edit' );
		...
		if ( $composite_container_item_key = wc_cp_get_composited_cart_item_container( $values, WC()->cart->cart_contents, true ) ) {
			...
			if ( apply_filters( 'woocommerce_add_composited_cart_item_subtotals', true, $values, $cart_item_key ) ) {
				...
				if ( ! empty( $children ) ) {
					...
					$composite_price        = 'excl' === get_option( 'woocommerce_tax_display_cart' ) ? WC_CP_Core_Compatibility::wc_get_price_excluding_tax( $values[ 'data' ], array( 'qty' => $values[ 'quantity' ] ) ) : WC_CP_Core_Compatibility::wc_get_price_including_tax( $values[ 'data' ], array( 'qty' => $values[ 'quantity' ] ) );

					foreach ( $children as $child_key => $child_data ) {
						$composited_item_price   = 'excl' === get_option( 'woocommerce_tax_display_cart' ) ? WC_CP_Core_Compatibility::wc_get_price_excluding_tax( $child_data[ 'data' ], array( 'qty' => $child_data[ 'quantity' ] ) ) : WC_CP_Core_Compatibility::wc_get_price_including_tax( $child_data[ 'data' ], array( 'qty' => $child_data[ 'quantity' ] ) );
						...
						if ( wc_cp_is_composite_container_cart_item( $cart_item_data ) ) {
							...
							if ( $composite->needs_shipping() ) {
								...
								foreach ( wc_cp_get_composited_cart_items( $cart_item_data, WC()->cart->cart_contents, true ) as $child_item_key ) {
									...
									if ( isset( $composited_product->composited_value ) ) {

										$bundled_value += $composited_product->composited_value * $composited_product_qty;
									...
									if ( isset( $composited_product->composited_weight ) ) {
										$bundled_weight += $composited_product->composited_weight * $composited_product_qty;
									}
								}
