    ...
    class YITH_WCP_Cart {
        ...
        public function add_cart_item( $cart_item ) {
            ...
            if( isset( $cart_item['yith_wcp_child_component_data'] ) ) {
                ...
                if( $child_item_meta['yith_wcp_component_parent_object']->isPerItemPricing() ) {
                    ...
                    YITH_WCP_Frontend::markProductAsCompositeProcessed(  $product , yit_get_base_product_id( $child_item_meta['yith_wcp_component_parent_object'] ), $child_item_meta['yith_wcp_component_key'] );

                }  else {

                    yit_set_prop( $product , 'price' , 0 );
                }
                        ...
                        if( isset( $component_data['selection_data'][$key] ) ) {
                            ...
                            if( $component_data['selection_data'][$key] > 0 ) {
                                ...
                                if( isset( $component_data['selection_variation_data'][$key] ) ) {
                                    ...
                                    // classic variation selected by the user
                                    ...
                                    // variation selection forced by the dependence
        ...
        public function cart_item_permalink( $url , $cart_item, $cart_item_key ) {
            ...
            if( isset( $cart_item[ 'yith_wcp_component_data' ] ) ) {
                ...
                $url= $url.'?ywcp_cart_item_key='.$cart_item_key;
        ...
        public function cart_item_name( $title, $cart_item, $cart_item_key ) {
             ...
             if( isset( $cart_item[ 'yith_wcp_child_component_data' ] ) ) {
                  ...
                  $component_name = apply_filters( 'ywcp_item_component_name' , $component_data[ 'name' ] , yit_get_base_product_id( $child_data[ 'yith_wcp_component_parent_object' ] ) , $child_data[ 'yith_wcp_component_key' ] ) ;
                   ...
                   $title = '<div class="ywcp_cart_component_name"><label>'.esc_html( $component_name ).':</label>'.$title.'</div>';
            }
        ...
        public function cart_item_price( $product_price , $cart_item ) {
            ...
            if( $product->get_type() == 'yith-composite' && !empty( $cart_item['yith_wcp_component_data'] ) && $cart_item['line_total'] == 0 ) {

                $product_price = '';

            } else if( !empty( $cart_item['yith_wcp_child_component_data'] ) && $cart_item['line_total'] == 0 ) {
    ...
    public function update_quantity_in_cart( $cart_item_key, $quantity = 0 ) {
                    ...
                    if( isset( WC()->cart->cart_contents[ $parent_key ] ) ) {
                        ...
                        if( isset( WC()->cart->cart_contents[ $parent_key ]['yith_wcp_component_data'] ) ) {
                            ...
                            if( $parent_quanity == 1 || $sold_individually ) {
                                ...
                                WC()->cart->cart_contents[ $parent_key ]['yith_wcp_component_data']['selection_quantity'][$wcp_child_key] = $quantity;
                ...
                if( $product->get_type() == 'yith-composite' && !empty( $cart_item['yith_wcp_component_data'] ) ) {
                    ...
                    if( $product->isPerItemPricing() ) {
                        ...
                        if( $composite_base_price > 0 ) {
                            $price_html .= '<div>('._x( 'Base Price:' , 'cart price advice' , 'yith-composite-products-for-woocommerce' ).' '.wc_price( $composite_base_price * $cart_item['quantity'] ).' + '._x( 'Total Price:' , 'cart price advice' , 'yith-composite-products-for-woocommerce'  ).' '.wc_price( $composite_total ).')</div>';
                        }
        ...
        public function order_item_subtotal( $product_sub_total , $item , $order ) {
            ...
            if( isset( $item['item_meta'] ) ) {
                ...
                if( $product->get_type() == 'yith-composite' && !empty( $cart_item['_yith_wcp_component_data'][0] ) ) {
                                ...
                                if( isset( $cart_child_meta_data['yith_wcp_cart_parent_key'] ) && $cart_child_meta_data['yith_wcp_cart_parent_key'] == $cart_item_key  ) {
                                    ...
                                    if(  $cart_child_meta_data['yith_wcp_component_key'] == $key ) {
                                        ...
                                        if( $sold_individually ) {
                                            ...
                                            $single_total =  yit_get_display_price( $child_product ) * $child_quantity * $global_quantity;
        ...
        public function order_again_cart_item_data( $cart_item_data, $item, $order ) {
            ...
            if ( isset( $item['item_meta']['_yith_wcp_component_data'][0] ) ) {
                $cart_item_data['yith_wcp_component_data'] = maybe_unserialize( $item['item_meta']['_yith_wcp_component_data'][0] );
            }
            else if( $item['item_meta']['_yith_wcp_child_component_data'][0] ) {
                $cart_item_data['yith_wcp_child_component_data'] = maybe_unserialize( $item['item_meta']['_yith_wcp_child_component_data'][0] );
                $cart_item_data['yith_wcp_child_component_data_no_reorder'] = $item['item_meta']['_yith_wcp_child_component_data_no_reorder'][0];
            }
                            ...
                            if ( isset( $cart_item_data[ 'yith_wcp_component_data' ] ) ) {
                                ...
                                if ( ! $composite->isPerItemShipping() ) {
                                    ...
                                    $bundle_totals = array(
                                        ...
                                        yit_set_prop( $composite , 'weight' , ( yit_get_prop( $composite , 'weight' ) + ( $bundled_weight / $composite_qty ) ) );
