...
<div class="ywcp_components_container ywcp_<?php echo esc_attr( $layout_options ) ?>" data-product-id="<?php echo $product_id; ?>" data-per-item-price="<?php echo ( $product_per_item_pricing ? 1 : 0 )?>" data-layout-option="<?php echo esc_attr( $layout_options ) ?>" data-dependencies="<?php echo esc_attr( json_encode( $product->getDependenciesDataComponentsDetails() ) ); ?>">
	...
	foreach ( $component_data as $key => $component_item ) {
		...
		echo '<div id="ywcp_component_'.$key.'" class="ywcp_components_single_item '.( $required ? 'ywcp_components_required' : '' ).' '.( $is_first_opened ? 'ywcp_selection_opened' : '' ).'" data-component-key="'.$key.'" data-required="'.$required.'" data-exclusive="'.$exclusive.'" data-title="'.esc_attr( $title ).'" data-sold-individually="'.$sold_individually.'">';
			...
			echo '<div class="ywcp_component_options_selection_container">';
...
<div class="ywcp_wcp_group_total" data-product-price="<?php echo esc_attr( $product_display_price ); ?>">
    <table>
        ...
        <tr><td><?php echo apply_filters( 'ywcp_product_base_price_text' , __( 'Product Base Price:' , 'yith-composite-products-for-woocommerce' ) ); ?></td><td><div class="yith_wcp_group_option_total_base_price"><?php echo wc_price( $product_display_price ) ?></div></td></tr>
        <?php endif; ?>
		<tr id="ywcp_wcp_tr_wapo_option_total" class="ywcp_wapo_total_hided"><td><?php _e( 'Add-Ons Option Total:' , 'yith-composite-products-for-woocommerce' ) ?></td><td><div class="yith_wcp_wapo_add_ons_total"><span class="woocommerce-Price-amount amount"></span></div></td></tr>
		<?php if( $product_per_item_pricing ): ?>
        <tr id="ywcp_wcp_tr_component_total"><td><?php _e( 'Component Options Total:' , 'yith-composite-products-for-woocommerce' ) ?></td><td><div class="yith_wcp_component_total"><span class="woocommerce-Price-amount amount"></span></div></td></tr>
		<?php endif; ?>
		<tr id="ywcp_wcp_tr_order_total"><td><?php _e( 'Order Total:' , 'yith-composite-products-for-woocommerce' ) ?></td><td><div class="yith_wcp_group_final_total"><span class="woocommerce-Price-amount amount"></span></div></td></tr>
    </table>
</div>
