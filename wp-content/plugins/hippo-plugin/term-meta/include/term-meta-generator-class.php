						...
						foreach ( $_POST as $post_key => $post_value ) {
							...
							if ( $field[ 'id' ] == $post_key ) {
								...
								switch ( $field[ 'type' ] ) {
									case 'text':
									case 'color':
										hippo_plugin_update_term_meta( $term_id, $field[ 'id' ], esc_html( $post_value ) );
										break;
									case 'url':
										hippo_plugin_update_term_meta( $term_id, $field[ 'id' ], esc_url( $post_value ) );
										break;
									case 'image':
										hippo_plugin_update_term_meta( $term_id, $field[ 'id' ], absint( $post_value ) );
										break;
									case 'textarea':
										hippo_plugin_update_term_meta( $term_id, $field[ 'id' ], esc_html( $post_value ) );
										break;
									case 'editor':
										hippo_plugin_update_term_meta( $term_id, $field[ 'id' ], wp_kses_post( $post_value ) );
										break;
									case 'select':
									case 'select2':
									case 'revslider':
										hippo_plugin_update_term_meta( $term_id, $field[ 'id' ], $post_value );
										break;
									default:
			...
			private function generate_form_fields( $term ) {
				...
				foreach ( $fields as $field ) {
					...
					if ( ! $term ) {
						...
						$field[ 'value' ] = hippo_plugin_get_term_meta( $term->term_id, $field[ 'id' ], TRUE );
					}
					...
					switch ( $field[ 'type' ] ) {
						...
						case 'revslider':
							...
							if ( class_exists( 'RevSlider' ) ) {
								...
								?>
								<select name="<?php echo $field[ 'id' ] ?>" id="<?php echo $field[ 'id' ] ?>"
								        class="hippo-plugin-select2" <?php echo $field[ 'multiple' ] ?>>
									<?php
										foreach ( $field[ 'options' ] as $key => $option ) {
											echo '<option' . selected( $field[ 'value' ], $key, FALSE ) . ' value="' . $key . '">' . $option . '</option>';
										}
								...
								<div class="error notice is-dismissible below-h2">
									<p><?php esc_html_e( 'Revolution Slider is not Installed or Activated.', 'hippo-plugin' ) ?></p>
									<button type="button" class="notice-dismiss"><span
											class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'hippo-plugin' ) ?></span>
									</button>
								</div>
								<?php
							}
