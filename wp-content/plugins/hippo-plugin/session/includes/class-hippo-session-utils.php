<?php
	...
	class Hippo_Session_Utils {
		...
		public static function count_sessions() {
			...
			$query = "SELECT COUNT(*) FROM $wpdb->options WHERE option_name LIKE '_hippo_session_expires_%'";
			 ...
			 */
			$query = apply_filters( 'hippo_session_count_query', $query );
		...
		public static function create_dummy_session( $date = NULL ) {
			...
			if ( NULL === $date ) {
				 ...
				 * @param int
				 */
				$expires = time() + (int) apply_filters( 'hippo_session_expiration', 30 * 60 );
			}

			$session_id = self::generate_id();

			// Store the session
			add_option( "_hippo_session_{$session_id}", array(), '', 'no' );
			add_option( "_hippo_session_expires_{$session_id}", $expires, '', 'no' );
		}
		...
		public static function delete_old_sessions( $limit = 1000 ) {
			...
			$limit = absint( $limit );
			$keys  = $wpdb->get_results( "SELECT option_name, option_value FROM $wpdb->options WHERE option_name LIKE '_hippo_session_expires_%' ORDER BY option_value ASC LIMIT 0, {$limit}" );
			...
			foreach ( $keys as $expiration ) {
				...
				if ( $now > $expires ) {
					...
					$expired[] = $key;
					$expired[] = "_hippo_session_{$session_id}";
		...
		public static function delete_all_sessions() {
			...
			$count = $wpdb->query( "DELETE FROM $wpdb->options WHERE option_name LIKE '_hippo_session_%'" );
