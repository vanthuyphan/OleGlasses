		...
		class ReduxFramework_hippo_preset extends ReduxFramework {
			...
			function __construct( $field = array(), $value = '', $parent ) {
				...
				if ( empty( $this->extension_dir ) ) {
					$this->extension_dir = trailingslashit( sprintf( '%2%s/redux-extensions/%1$s/%1$s', $this->field[ 'type' ], untrailingslashit( HIPPO_PLUGIN_PATH ) ) );
					$this->extension_url = trailingslashit( sprintf( '%2$s/redux-extensions/%1$s/%1$s', $this->field[ 'type' ], untrailingslashit( HIPPO_PLUGIN_URL ) ) );
				}
			...
			public function render() {
				...
				if ( ! empty( $this->field[ 'options' ] ) ) {
					...
					do_action( 'hippo_preset_field_before', $this );
					...
					foreach ( $this->field[ 'options' ] as $k => $v ) {
						...
						$image = get_template_directory_uri() . '/img/presets/' . $k . '.png';
					...
					echo '</ul>';
					echo '</div>';
					do_action( 'hippo_preset_field_after', $this );
				}
